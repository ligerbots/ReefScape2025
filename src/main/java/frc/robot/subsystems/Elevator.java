...
public class Elevator extends SubsystemBase {
    public static final double NARROW_PIVOT_MAX_HEIGHT = Units.inchesToMeters(12.0);
    public static final double SLOW_PIVOT_MIN_HEIGHT = Units.inchesToMeters(48.0);
    ...
    // voltage needed to offset gravity
    ...
    public void periodic() {
        ...
        if (m_goalMetersClipped < MIN_HEIGHT_TURN_OFF && height < MIN_HEIGHT_TURN_OFF) {
            ...
            m_motor.setControl(new MotionMagicVoltage(heightToRotations(m_goalMetersClipped)));
        }
        
        SmartDashboard.putNumber("elevator/height", Units.metersToInches(height));
        SmartDashboard.putNumber("elevator/clippedGoal", Units.metersToInches(m_goalMetersClipped));
    ...
    public boolean lengthWithinTolerance() {
        return Math.abs(getHeight() - m_goalMetersClipped) < LENGTH_TOLERANCE_METERS;
    }
    ...
    private static double limitElevatorLength(double length, boolean pivotOutsideLowRange) {
        ...
        if (!pivotOutsideLowRange) {
            clampedHeight = MathUtil.clamp(length, MIN_LENGTH_METERS, MAX_LENGTH_METERS);
        } else {
